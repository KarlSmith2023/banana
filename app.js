...
(function () {
  ...
  var LS_KEY = "desk-static-book-v1";
  var SEED_AS_OF = "2026-09-11";
  var DEFAULT_FX = 129.26;
  ...
  var SEED_SETUPS = [
    { symbol: "KPL", name: "KPL", last: 3643.3, changePct: -0.5, pivot: 3661.6, rs: 98, tags: ["Forming", "BlueSky"], note: "Forming watch — <=2% from pivot candidate" },
    { symbol: "BLUESTONE", name: "Bluestone Jewellery", last: 890.35, changePct: -0.5, pivot: 894.8, rs: 90, tags: ["Forming", "BlueSky"], note: "Forming watch" },
    { symbol: "CPPLUS", name: "CP Plus / Aditya Infotech", last: 3825.5, changePct: -1.9, pivot: 3899.1, rs: 96, tags: ["Forming", "VCP\u2229BlueSky", "VCP", "BlueSky"], note: "Forming — VCP\u2229Blue sky, -1.9% from pivot 3899.1" },
    { symbol: "DIACABS", name: "Diamond Power / Dia Cables", last: 369.05, changePct: 0.1, pivot: 368.7, rs: 97, tags: ["Fresh", "BlueSky"], note: "Fresh proposed — BO 11 Sep" },
    { symbol: "FILATEX", name: "Filatex India", last: 87.41, changePct: 0.8, pivot: 86.75, rs: 92, tags: ["Fresh", "VCP\u2229BlueSky", "VCP", "BlueSky"], note: "Fresh proposed — BO 11 Sep VCP\u2229Blue sky" },
    { symbol: "KABRAEXTRU", name: "Kabra Extrusion", last: 568.0, changePct: 13.7, pivot: 499.5, rs: 88, tags: ["DoNotChase", "BlueSky"], note: "Do-not-chase example +13.7% above pivot" },
    { symbol: "WHEELS", name: "Wheels India", last: 1124.0, changePct: 24.9, pivot: 900.0, rs: 86, tags: ["DoNotChase"], note: "Do-not-chase example +24.9%" },
  ];
  ...
  function inr(n, digits) {
    ...
    var neg = n < 0;
    var abs = Math.abs(n);
    var fixed = abs.toLocaleString("en-IN", { minimumFractionDigits: digits, maximumFractionDigits: digits });
    return (neg ? "-" : "") + "\u20B9" + fixed;
  }
  function gbp(n, fx, digits) {
    if (digits == null) digits = 2;
    var v = n / (fx || DEFAULT_FX);
    var neg = v < 0;
    var abs = Math.abs(v);
  ...
  function nowIst() {
    var s = new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" });
    return new Date(s);
  }
  ...
  function sessionLabel(simSession, d) {
    d = d || nowIst();
    var open = isNseOpen(d);
    var weekend = !isWeekdayIst(d);
    var isSim = !!(simSession || weekend);
    var label = "CLOSED";
    if (isSim && open) label = "SIM \u00B7 OPEN";
    else if (isSim) label = "SIM \u00B7 DEMO";
    else if (open) label = "NSE OPEN";
    else if (isWeekdayIst(d)) label = "NSE CLOSED";
    else label = "WEEKEND";
    return { label: label, isSim: isSim, isOpen: open || isSim, sessionDate: formatIstDate(d) };
  }
  ...
  function computeSize(equity, entry, settings, stopPct) {
    ...
    var riskInr = equity * (settings.riskPctPerIdea / 100);
    var stopDist = entry * (stop / 100);
    if (stopDist <= 0) return 0;
    return Math.floor(riskInr / stopDist);
  }
  ...
  function getQuote(symbol) {
    ...
    var now = new Date().toISOString();
    return { symbol: sym, last: 100, bid: 99.95, ask: 100.05, changePct: 0, source: "mark", updatedAt: now };
  }
  ...
  function applyFill(order, qty, price, slipBps) {
    ...
    if (order.side === "BUY") {
      ...
      if (pos) {
        ...
        var riskInr = (settings.riskPctPerIdea / 100) * markToMarketEquity();
        book.positions.push({
      ...
      if (p2) {
        ...
        var rMult = p2.entryRiskInr > 0 ? pnl / p2.entryRiskInr : null;
        if (remain <= 1e-9) {
  ...
  function placeOrder(input) {
    var settings = getSettings();
    var symbol = String(input.symbol || "").toUpperCase();
    ...
    if (input.side === "BUY" && notional > account.buyingPower + 0.01) {
      return { ok: false, error: "Insufficient buying power (need " + inr(notional, 0) + ", have " + inr(account.buyingPower, 0) + ")" };
    }
    if (input.side === "SELL") {
  ...
  function ingestSetups(payload) {
    var asOf = payload.asOf || formatIstDate();
  ...
  function renderNav() {
    ...
    if (dateEl) dateEl.textContent = formatIstDate();
  ...
  function viewOverview() {
    ...
    var posHtml;
    if (!positions.length) {
      posHtml = '<p class="empty">No open positions. Try paper-buying <a href="#/ticket?symbol=FILATEX">FILATEX</a> from the ticket.</p>';
    } else {
  ...
  function viewTicket() {
    ...
    return (
      ...
      '<div class="sub">' + esc(setup && setup.name ? setup.name : "cash equity") + ' · seed mark · India cash long-only</div>' +
      '</div><div class="tr-px">' +
      ...
      '</div>' +
      '<div class="banner warn-banner" style="margin-bottom:10px"><b>India cash · long-only</b> — Short / leverage N/A on paper cash. Sell reduces an open long.</div>' +
      ...
      '<div class="halt-cap"><b>live locked</b> · ' + esc(bits.liveLabel) + ' · PLACE PAPER ORDER only (not live)</div>' +
      (ticketState.msg ? '<div class="msg ' + (okMsg ? "ok" : "err") + '" style="margin-top:8px">' + esc(ticketState.msg) + "</div>" : "") +
      '<div class="sub" style="margin-top:8px" id="tr-msg">Attached stop (' + settings.stopPct + '%) + trail on filled BUY · Outside NSE hours market queues for next open · Quotes are seed marks · Not financial advice</div>' +
      '</div>' +
      ...
      '<div class="bal" style="background:var(--panel2);border:1px solid var(--line);border-radius:8px;padding:10px"><div class="k" style="font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:var(--faint)">Last</div><div class="v num" style="font-size:14px;margin-top:4px;font-weight:600">' + last.toFixed(2) + '</div></div>' +
      ...
      '<p class="sub" style="margin-top:12px">Paper engine · £800 start (₹1,03,408 @ 129.26) · 60-session live lock · localStorage</p>' +
      '</div></div></div>'
    );
  }
  ...
  function viewPositions() {
    ...
    var positions = getPositions().map(function (p) {
      var upl = (p.last - p.avgPrice) * p.qty;
      var r = p.entryRiskInr > 0 ? upl / p.entryRiskInr : null;
    ...
    var posBody;
    if (!positions.length) {
      posBody = '<p class="empty">Flat. <a href="#/ticket?symbol=FILATEX">Paper-buy FILATEX</a></p>';
    } else {
  ...
  function viewCharts() {
    ...
    var chips = ["FILATEX", "DIACABS", "CPPLUS", "KPL", "BLUESTONE"].map(function (s) {
  ...
  function viewPlaybook() {
    ...
    return (
      ...
      '<p class="muted" style="font-size:12px;margin:0">Live stays hard-locked until ' + s.liveGateSessionsRequired + " NSE trading sessions are recorded <em>and</em> the checklist below is ticked. Live itself is a stub \u2014 connect a broker after paper confirmation.</p>" +
  ...
  function bindPlaybook() {
    ...
    if (rst) rst.addEventListener("click", function () {
      if (!confirm("Reset paper book to seed (\u00A3800 / \u20B91,03,408, 11 Sep 2026 setups)? This clears positions, orders, and sessions.")) return;
      resetBook();
  ...
  function bindTicket() {
    ...
    if (sym) {
      sym.addEventListener("input", function () { ticketState.symbol = sym.value.toUpperCase(); });
      sym.addEventListener("blur", function () { ticketState.symbol = sym.value.toUpperCase(); go("ticket", { symbol: ticketState.symbol }); });
    }
  ...
  function computeNextMove() {
    ...
    if (!actionables.length) {
      return {
        action: "WAIT",
        title: "Next move",
        body: "No actionable Fresh/Forming names inside chase + RS gates.\n\nKeep paper. Reassess scanner after next NSE close.\nDo-not-chase examples (KABRAEXTRU / WHEELS) stay blocked.",
  ...
  if (hasDom) {
    ...
    if (!location.hash) location.hash = "#/ticket?symbol=FILATEX";
    render();
    registerSW();
